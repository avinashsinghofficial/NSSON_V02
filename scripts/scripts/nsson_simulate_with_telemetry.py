...
# =========================================================
# Jetson-based model defaults (loaded from JSON)
# =========================================================
JETSON_BASES_PATH = Path("jetson_emulation") / "jetson_bases.json"

if not JETSON_BASES_PATH.exists():
    raise RuntimeError(
        f"Jetson base file not found: {JETSON_BASES_PATH}. "
        f"Run jetson_emulation/calibrate_bases.py first."
    )

JETSON_BASES = json.loads(JETSON_BASES_PATH.read_text())

def model_defaults(model):
    model = model.lower()
    if model not in JETSON_BASES:
        raise ValueError(f"Unsupported model (no Jetson bases): {model}")
    return JETSON_BASES[model]

# =========================================================
# Architecture and profile factors (same as before)
# =========================================================
...
# =========================================================
# NSSON simulation using Jetson bases
# =========================================================
def simulate_run(model, nodes, architecture, profile, cycles, seed):
    random.seed(seed)
    np.random.seed(seed)

    md = model_defaults(model)          # Jetson-derived base values
...
def main():
    parser = argparse.ArgumentParser(description="NSSON telemetry simulator using Jetson-based model defaults")
