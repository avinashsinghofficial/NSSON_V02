...
Implemented behavior:
  - TCP/8090: autonomous slice, queue 1, highest priority.
  - TCP/8091: healthcare slice, queue 2.
  - TCP/8092: industrial slice, queue 3.
  - UDP/5001: background best-effort traffic, queue 0.
  - OpenFlow 1.3 learning-switch forwarding.
  - JSONL audit records for every classified flow.

This controller is designed for the validated two-switch topology:
  qc + qb1 -- s1 -- s2 -- qs + qb2

The current controller provides per-slice classification and queue selection.
Adaptive routing will be implemented separately after the multi-slice SLA
experiment is validated with the current topology.
"""
...
class NSSONMultiSliceSLAController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.tcp_port_to_slice = {
            int(slice_cfg["port"]): slice_name
            for slice_name, slice_cfg in self.slices.items()
            if str(slice_cfg.get("protocol", "")).lower() == "tcp"
        }

        self.logger.info(
            "NSSON multi-slice SLA controller started: TCP slice ports=%s",
            self.tcp_port_to_slice,
        )

    def classify_flow(self, pkt):
        """
        Classify traffic into autonomous, healthcare, industrial, or background.
        """
        tcp_packet = pkt.get_protocol(tcp.tcp)
        udp_packet = pkt.get_protocol(udp.udp)

        if tcp_packet is not None:
            slice_name = self.tcp_port_to_slice.get(
                int(tcp_packet.dst_port),
                "background",
            )
        ...
        if udp_packet is not None:
            slice_cfg = self.slices["background"]
        ...
        record.update(
            {
                ...
                "policy": "per_slice_queue_and_sla",
                "path_id": "single_path",
                "selected": 1,
            }
        )
    ...
    def switch_features_handler(self, event):
        ...
        self.logger.info(
            "Installed table-miss rule: dpid=%s",
            datapath.id,
        )
