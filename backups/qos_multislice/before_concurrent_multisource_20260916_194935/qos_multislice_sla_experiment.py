...
Controller modes:
    baseline_sdn_iot : all flows use best-effort queue 0.
    priority_only    : controller flow priorities, but no OVS queue reservation.
    slicing_sla      : controller priorities plus per-slice OVS HTB queues.

Measurement safeguards:
    - ARP/MAC/OpenFlow/queue warm-up for every slice.
    - Reproducibly randomized slice measurement order per repetition.
    - Real Mininet ping and iperf measurements only.
    - One CSV row per slice, load, mode, and repetition.

Run:
    sudo /usr/bin/python3 \
      /home/avi/NSSON_V02/qos_multislice/qos_multislice_sla_experiment.py \
      --mode slicing_sla --load light --rep 101 --controller-port 6633
"""
...
def safe_cleanup():
    """Remove only experiment-created bridges and temporary test processes."""
    for bridge in ("s1", "s2"):
...
def parse_ping(output):
    """Parse standard Linux ping output."""
    tx = 0
    rx = 0
    loss_pct = 100.0
    rtt_min = float("nan")
    ...
    if packet_match:
        tx = int(packet_match.group(1))
        rx = int(packet_match.group(2))
        loss_pct = float(packet_match.group(3))
...
def parse_iperf_bandwidth(output):
    """Parse the final bandwidth value from classic iperf output."""
...
def append_row(csv_path, row):
    """Append one metrics row, writing CSV headers only for a new file."""
...
def get_bottleneck_interface(switch_name):
    """
    Return the known s1--s2 bottleneck interface.

    Links are added in fixed order:
      s1-eth1 <-> qc
      s1-eth2 <-> qb1
      s1-eth3 <-> s2
...
def configure_ovs_queues(interface_name, mode):
    """
    Configure Linux-HTB queues on an OVS bottleneck-facing port.

    Queue mapping:
      0: best-effort UDP background
      1: autonomous inference
      2: healthcare inference
      3: industrial inference

    Note:
      The ``--`` separator is required before each ovs-vsctl transaction.
    """
    if mode != "slicing_sla":
        return
...
def compute_sla(slice_cfg, metrics):
    ...
    V_k(t) = I[
        RTT_k > RTT_k^max OR
        J_k > J_k^max OR
        loss_k > loss_k^max OR
        PDR_k < PDR_k^min OR
        throughput_k < throughput_k^min
    ].
    """
    sla = slice_cfg["sla"]

    rtt_ok = metrics["rtt_avg_ms"] <= float(sla["rtt_ms_max"])
    jitter_ok = (
        metrics["rtt_jitter_mdev_ms"] <= float(sla["jitter_ms_max"])
    )
    ...
    all_ok = rtt_ok and jitter_ok and loss_ok and pdr_ok and throughput_ok

    return {
        "sla_rtt_met": int(rtt_ok),
...
def start_servers(server, bg_sink):
    """Start one TCP iperf server per inference slice and UDP background server."""
...
def warm_up_slice_flows(client, server):
    """
    Warm ARP, MAC-learning, table-miss forwarding, OpenFlow rules, and queues
    for every inference port before any QoS measurement is collected.
    """
    info("*** Warming up ARP, MAC-learning, OpenFlow rules, and queues\n")

    client.cmd(
        f"ping -c 3 -i 0.1 -W 1 {server.IP()} "
        ">/tmp/nsson_qosms_warmup_ping.log 2>&1"
    )
    ...
    for slice_name in ("autonomous", "healthcare", "industrial"):
        ...
        client.cmd(
            ...
            f"-t {WARMUP_SECONDS} -i 1 "
            f">/tmp/nsson_qosms_warmup_{slice_name}.log 2>&1"
        )

    time.sleep(POST_WARMUP_SETTLE_SEC)


def run_experiment(mode, load_name, rep, controller_port):
    """Create topology, configure QoS, collect measured metrics, and clean up."""
    valid_modes = ("baseline_sdn_iot", "priority_only", "slicing_sla")

    if mode not in valid_modes:
        raise ValueError(
            f"Unsupported mode={mode}. Choose one of {valid_modes}."
        )
    ...
    background_rate_mbps = float(load_cfg["background_mbps"])
    ...
    s1_bottleneck_interface = get_bottleneck_interface("s1")
    s2_bottleneck_interface = get_bottleneck_interface("s2")

    if mode == "slicing_sla":
        configure_ovs_queues(s1_bottleneck_interface, mode)
        configure_ovs_queues(s2_bottleneck_interface, mode)

    start_servers(server, bg_sink)

    for slice_name in ("autonomous", "healthcare", "industrial"):
        port = int(SLICES[slice_name]["port"])

        reachability = client.cmd(
            f"nc -zvw 2 {server.IP()} {port} 2>&1"
        )

        info(
            f"*** {slice_name} TCP/{port} reachability: "
            f"{reachability}"
        )

    warm_up_slice_flows(client, server)

    background_port = int(SLICES["background"]["port"])

    info(
        f"*** Starting UDP background load: "
        f"{background_rate_mbps:.0f} Mbit/s\n"
    )

    bg_source.cmd(
        f"iperf -c {bg_sink.IP()} -u -p {background_port} "
        f"-b {background_rate_mbps:.0f}M "
        f"-t {DURATION_SEC} -i 1 "
    ...
    # Reproducible randomized order prevents one slice from systematically
    # being measured immediately after flow installation.
    random.Random(rep).shuffle(slice_order)

    info(f"*** Measurement order: {slice_order}\n")
    ...
    with open(text_path, "w", encoding="utf-8") as text_file:
        text_file.write(
            ...
            f"MEASUREMENT_ORDER={slice_order}\n"
        )

        for measurement_index, slice_name in enumerate(slice_order, start=1):
            slice_cfg = SLICES[slice_name]
            port = int(slice_cfg["port"])

            info(
                f"*** Measuring {slice_name} slice "
                f"on TCP/{port}, order={measurement_index}\n"
            )

            ping_output = client.cmd(
                f"ping -c {PING_COUNT} "
                f"-i {PING_INTERVAL_SEC} "
                f"-W 1 {server.IP()}"
            )
            ...
            tcp_throughput_mbps = parse_iperf_bandwidth(tcp_output)
            ...
            row = {
                ...
                "rep": rep,
                "slice": slice_name,
                "measurement_order": measurement_index,
...
def main():
    parser = argparse.ArgumentParser(
        description="Measured multi-slice QoS and SLA experiment for NSSON."
    )
