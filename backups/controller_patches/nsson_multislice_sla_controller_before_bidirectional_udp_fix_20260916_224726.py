#!/usr/bin/env python3
"""
NSSON multi-slice SLA-aware Ryu controller with UDP QoS probes.
...
The UDP probe ports allow jitter, packet-loss, and PDR measurements to be
classified into the same logical queue as their associated inference slice.
"""
...
class NSSONMultiSliceSLAController(app_manager.RyuApp):
    ...
    def slice_record(self, slice_name, protocol, src_port, dst_port):
    ...
    def classify_flow(self, pkt):
        ...
        if udp_packet is not None:
            slice_name = self.udp_port_to_slice.get(
                int(udp_packet.dst_port),
                "background",
            )
        ...
        record.update(
            {
                ...
                "policy": "per_slice_queue_sla_udp_probe",
                "path_id": "single_path",
                "selected": 1,
            }
        )
