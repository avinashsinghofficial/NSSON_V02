...
def parse_bandwidth_mbps(output):
    """
    Compute a robust TCP throughput statistic from classic iperf output.

    Classic iperf frequently ends with a zero-throughput teardown interval.
    Returning the final matching line would therefore underestimate throughput.
    We retain active intervals of at least 0.5 seconds and report their median,
    which is robust to TCP startup, shutdown, and short scheduler disturbances.
    """
    active_rates = []
    ...
    for line in output.splitlines():
        ...
        # Remove terminal/idle records. For this topology, 1 Kbit/s is safely
        # below a meaningful active inference-flow rate.
        if rate_mbps > 0.001:
